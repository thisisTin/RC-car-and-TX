...
#define Jst1 1  // Joystick button 1
#define Jst2 0  // Joystick button 2
#define sw1 7   // Toggle switch 1
#define sw2 4   // Toggle switch 1
#define bt1 8   // Button 1
#define bt2 9
...
void setup() {
  ...
  pinMode(Jst1, INPUT_PULLUP);
  pinMode(Jst2, INPUT_PULLUP);
...
void loop() {
  ...
  data.j1Button = digitalRead(Jst1);
  data.j2Button = digitalRead(Jst2);
  data.tSwitch2 = digitalRead(sw2);
  data.button1 = digitalRead(bt1);
  data.button2 = digitalRead(bt2);
...
void read_IMU() {
  ...
  // Currently the raw valueFds are in degrees per seconds, deg/s, so we need to multiply by sendonds (s) to get the angle in degrees
